...
// 2.1.2：修复 INI 读回改写时 UTF-8 BOM 逐次累积（CI 以 TestIniSetKeysDeterministic 抓到；
//
//	重复安装/卸载后 ReShade 配置文件头部会堆积 BOM）；GUI/version.dll 探测加
//	//go:build windows 标签并新增 stub_other.go，使 go test ./... 可跨平台执行
...
// stripBOM 剥离文本开头的 UTF-8 BOM（含历史版本累积的多个 BOM）。
// writeTextBOM 每次写入都会在开头补一个 BOM，读回改写前必须先剥离，
// 否则每保存一次就多一个 BOM（CI 曾以 TestIniSetKeysDeterministic 抓到该问题）。
